...
static void clear_screen(uint8_t ft)
{
    for (uint8_t y = 0; y < 18; y++) {
        wallet_font_clear_row(y, ft);
    }
}

static void draw_hline(uint8_t y, uint8_t ft)
{
    uint8_t dash = wallet_font_char_tile('-', ft);
...
{
    wallet_font_print_centered(0, title, ft);
    draw_hline(1, ft);
}
...
void ui_draw_splash(uint8_t ft)
{
    ...
    wallet_font_print_centered(6, "Stealth Wallet", ft);
    wallet_font_print_centered(8, "for Game Boy", ft);
    wallet_font_print_centered(11, "v0.1.0", ft);
    wallet_font_print_centered(16, "Press any key", ft);
}
...
void ui_draw_home(const ui_context_t *ctx, uint8_t ft)
{
    ...
    for (uint8_t i = 0; i < HOME_ITEM_COUNT; i++) {
        uint8_t y = 3 + i;
        if (i == ctx->cursor) {
            wallet_font_print(1, y, ">", ft);
            wallet_font_print(3, y, items[i], ft);
        } else {
            wallet_font_print(3, y, items[i], ft);
        }
    }
...
void ui_draw_pin_entry(const ui_context_t *ctx, uint8_t ft, const char *title)
{
    ...
    wallet_font_print_centered(6, "Enter PIN:", ft);
    ...
    for (uint8_t i = 0; i < ctx->max_pin_pos; i++) {
        ...
        if (i < ctx->pin_length) {
            ...
            char buf[2] = {'*', '\0'};
            wallet_font_print(x, 9, buf, ft);
        } else if (i == ctx->pin_pos) {
            /* Current position: show cursor */
            wallet_font_print(x, 9, "_", ft);
        } else {
            /* Empty position */
            wallet_font_print(x, 9, "-", ft);
        }
    }

    /* Digit value at current position */
    char digit_str[2] = {'0' + ctx->pin_digits[ctx->pin_pos], '\0'};
    wallet_font_print_centered(12, digit_str, ft);

    wallet_font_print_centered(14, "U/D:digit R:next", ft);
    wallet_font_print_centered(15, "A:confirm B:back", ft);
}
                       ...
                       const char *word_text)
{
    ...
    wallet_font_print_centered(5, header, font_tiles);

    /* Show the word (large, centered) */
    if (word_text) {
        wallet_font_print_centered(8, word_text, font_tiles);
    }

    wallet_font_print_centered(14, "< prev  next >", font_tiles);
    wallet_font_print_centered(16, "B: done", font_tiles);
}
...
void ui_draw_address(const ui_context_t *ctx, uint8_t ft)
{
    ...
    for (uint8_t line = 0; line < 4 && offset < strlen(addr); line++) {
        ...
        chunk[len] = '\0';
        wallet_font_print(0, y + line, chunk, ft);
        offset += 20;
    }

    /* Scroll indicators */
    if (ctx->scroll_offset > 0) {
        wallet_font_print(19, 3, "^", ft);
    }
    if (offset < strlen(addr)) {
        wallet_font_print(19, 6, "v", ft);
    }

    wallet_font_print_centered(14, "U/D: scroll", ft);
    wallet_font_print_centered(16, "B: back", ft);
}
...
void ui_draw_tx_confirm(const ui_context_t *ctx, uint8_t ft)
{
    ...
    wallet_font_print(0, 3, "Send:", ft);
    /* Show amount (from data_buf, assumed formatted in text_buf) */
    wallet_font_print(0, 4, ctx->text_buf, ft);

    wallet_font_print(0, 6, "To:", ft);
    ...
    wallet_font_print(0, 7, addr_short, ft);

    wallet_font_print(0, 10, "Fee:", ft);
    wallet_font_print(5, 10, ctx->text_buf + 40, ft);

    wallet_font_print_centered(14, "A: SIGN  B: REJECT", ft);
    wallet_font_print_centered(16, "Start: details", ft);
}
...
void ui_draw_settings(const ui_context_t *ctx, uint8_t ft)
{
    ...
    wallet_font_print(3, 3, "Network:", ft);
    wallet_font_print(13, 3, ctx->network == 0 ? "Mainnet" : "Testnet", ft);

    wallet_font_print(3, 5, "Wipe Wallet", ft);
    wallet_font_print(3, 7, "Show xpub", ft);
    wallet_font_print(3, 9, "Back", ft);
    ...
    wallet_font_print(1, cursor_y, ">", ft);
}
...
void ui_draw_xpub(const ui_context_t *ctx, uint8_t ft)
{
    ...
    wallet_font_print_centered(16, "U/D:scroll B:back", ft);
}
...
void ui_draw_success(const char *msg, uint8_t ft)
{
    ...
    wallet_font_print_centered(8, msg ? msg : "Done!", ft);
    wallet_font_print_centered(14, "Press any key", ft);
}
...
void ui_draw_error(const char *msg, uint8_t ft)
{
    clear_screen(ft);
    draw_title("= ERROR =", ft);
    wallet_font_print_centered(8, msg ? msg : "Unknown error", ft);
    wallet_font_print_centered(14, "Press any key", ft);
}
...
void ui_draw_loading(const char *msg, uint8_t ft, uint8_t anim_frame)
{
    ...
    wallet_font_print_centered(8, msg ? msg : "Please wait...", ft);

    /* Simple spinner */
    const char *spinner[] = {"|", "/", "-", "\\"};
    wallet_font_print_centered(10, spinner[anim_frame & 3], ft);
}
...
void ui_draw(const ui_context_t *ctx, uint8_t font_tiles)
{
    switch (ctx->state) {
        ...
        case UI_STATE_WIPE_CONFIRM:
            ...
            wallet_font_print_centered(6, "This will erase", font_tiles);
            wallet_font_print_centered(7, "all wallet data!", font_tiles);
            wallet_font_print_centered(10, "A: confirm", font_tiles);
            wallet_font_print_centered(11, "B: cancel", font_tiles);
            break;
